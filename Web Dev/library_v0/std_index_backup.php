			...
			<div class="frame" id="centered">
				...
				for($i =0; $i < count($book1); $i++){
					...
					<a href="std_detail.php?page=main&cat=000&b_no=<?= $item['book_id'];?>" >
						<img style="margin-top:50px;" width="160" src="<?=$item['file_path'] ;?><?=$item['cover'] ;?>" />
						</a>
...
<h2 style="background-image:url(images/cap_journal.png);background-repeat:no-repeat; color:white; padding-left:30px;padding-top:10px; height:60px;">วารสาร <small class="pull-right " style="margin-top:20px; font-size:14px;"> <a href="std_category.php?page=main&filter=2&cat=<?=@$_GET['cat'];?>">[ ดูทั้งหมด ]</a> </small></h2>
 ...
 <h2 style="background-image:url(images/cap_journal.png);background-repeat:no-repeat; color:white; padding-left:30px;padding-top:10px; height:60px;">Audio <small class="pull-right " style="margin-top:20px; font-size:14px;"> <a href="std_index.php?page=main&filter=4&cat=<?=@$_GET['cat'];?>">[ ดูทั้งหมด ]</a> </small></h2>
				...
				<!-- left panel -->
                    ...
                    <div style="float:left; width:784px; padding:20px 0px 0px 20px; border-top-style:dashed">
						...
						<div style="margin-bottom:30px;">
							...
							<div style="float:left; margin-top:14px; margin-left:100px; margin-bottom:10px;">
								...
								if($_p < 2){
									...
									echo '<li style="float:left; list-style:none; padding:10px;"><a href="std_index.php?page=main&filter=1&cat='.@$_GET['cat'].'&_p='. ($_p-1) .'"> <img src="images/left_arrow.png" style="border:0px; width:25px;" alt="left" /> </a></li>';
								}
									for($i = 0; $i < ceil($all_book_num_rows / $display_per_page); $i ++){
										...
										if(($_p-1) == $i){
											...
											echo '<li style="color:#cecece; float:left; list-style:none; padding:10px;"><a href="std_index.php?page=main&filter=1&cat='.@$_GET['cat'].'&_p='. ($i+1) .'">'. ($i+1) .'</a></li>';
										}														
									}
													
								if($_p == ceil($all_book_num_rows / $display_per_page)){
									//echo '<li style="float:left; list-style:none; padding:10px;"> ถัดไป </li>';
								}else{
									echo '<li style="float:left; list-style:none; padding:10px;"><a href="std_index.php?page=main&filter=1&cat='.@$_GET['cat'].'&_p='. ($_p+1) .'"> <img src="images/right_arrow.png" style="border:0px; width:25px;" alt="right" /> </a></li>';
								}
						...
						<div style="">
								...
								<div style="padding:2px; border:solid thin #ececec; width:166px; height:322px; float:left; margin-right:20px; margin-bottom:20px;">
								
									<img style="width:160px; height:220px;" src="<?=$item['file_path'] ;?><?=$item['cover'] ;?>" alt="<?=$item['bookname'] ;?>"> 
									<div style="font-weight:700; padding:4px; text-align:center;"><h4 style="word-wrap:break-word;">
									<a href="std_detail.php?page=main&amp;cat=<?=$item['category_id'] ;?>&category_name=<?=$value['category_name'];?>&b_no=<?=$item['book_id'] ;?>"><?=$item['bookname'] ;?></a>
									</h4></div>
