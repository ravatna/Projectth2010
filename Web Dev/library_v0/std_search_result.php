...
$bookdata = selects("tb_book", " where bookname like  '%".@$_GET['s']."%'  limit ". ($display_per_page * ($_p-1)) .",". $display_per_page);

 
	$data = selects("tb_book", " where bookname like  '%".@$_GET['s']."%'  limit ". ($display_per_page * ($_p-1)) .",". $display_per_page);	
...
<a href="std_category.php?page=main&cat=<?=@$value['category_id'];?>&filter=<?=((@$_GET['filter']) == "" ? "1" : $_GET['filter']);?>&category_name=<?=$value['category_name'];?>"><?=$value['category_name'];?></a>
                    ...
                    <div style="float:left; width:784px; padding:20px 0px 0px 20px; border-top-style:dashed">
						...
						<div style="margin-bottom:30px;">
							<h3 style="float:left;">ผลการค้นหา: <?=@$_GET['s'];?></h3>
							<div style="float:left; margin-top:14px; margin-left:100px; margin-bottom:10px;">
								...
								if($_p < 2){
									...
									echo '<li style="float:left; list-style:none; padding:10px;"><a href="std_category.php?page=main&filter='.((@$_GET['filter']) == "" ? "1" : $_GET['filter']).'&cat='.((@$_GET['cat']) == "" ? "000" : $_GET['cat']).'&category_name='.((@$_GET['category_name']) == "" ? "หนังสือพระราชนิพนธ์" : $_GET['category_name']).'&_p='. ($_p-1) .'"> <img src="images/left_arrow.png" style="border:0px; width:25px;" alt="left" /> </a></li>';
								}								 
